...
function filter(arr, idx1, idx2, mode, date1, date2) {
	...
	const out = arr.filter( (row) => {
			...
			if (mode == 'før' || mode == 'etter') {
				...
				const overlap = temporalOverlap(begin, end, dateA, dateB);
...
function begin() {
	...
	document.addEventListener(eName, () => {
			...
			b.onclick = () => {
					show(spinner);
					const colA = fxcd('begin-column').value;
					const colB = fxcd('end-column').value;
					const dateA = fxcd('begin').value;
					const dateB = fxcd('end').value;
					const mode = fxcd('mode-select').value.toLowerCase();
					
					outputCSV = filter(inputCSV, colA, colB, mode, dateA, dateB);
					
					const d = fxcd('download');
					d.disabled = false;
	...
	let dataReady = new Proxy(dataReadyTarget, {
			set: (target, key, value) => {
					...
					if (dataReady['fileA'] + dataReady['mode'] + dataReady['colA'] + dataReady['colB'] + dataReady['dateA'] == 0) {
						const v = fxcd('mode-select').value;
	...
	fxcd('file').onchange = () => {
			...
			if (file.files.length >= 1) {
				...
				r.onload = () => {
						...
						const a = fxcd('begin-column');
						const b = fxcd('end-column');
						a.innerHTML = '';
						b.innerHTML = '';
